...
describe("Native Auth", () => {
  ...
  const TTL = 86400;
  const ORIGIN = 'https://api.dharitri.com';
  const TOKEN = `aHR0cHM6Ly9hcGkuZGhhcml0cmkuY29t.${BLOCK_HASH}.${TTL}.e30`;
  ...
  describe("Client", () => {
    ...
    it("/blocks/latest throws error should fallback to /blocks?size=1", async () => {
      ...
      mock.onGet(`https://api.dharitri.com/blocks?size=1&fields=hash`).reply(200, [{ hash: BLOCK_HASH }]);
      const token = await client.initialize();
...
describe("Native Auth with gateway", () => {
  ...
  describe("Client", () => {
    it("Latest block should return signable token", async () => {
      ...
      onNetworkStatus(mock).reply(200, { data: { status: { moa_current_round: LATEST_ROUND } } });
